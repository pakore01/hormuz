...
function fetchRiesgoElectrico(){
  ...
  .then(function(d){
    if(d.riesgo_apagon !== undefined){
      /* Update energia centro */
      updateEnergiaRiesgo(d);
...
function fetchEURUSD(){
  if(!WORKERS.eurusd) return;
  fetch(WORKERS.eurusd)
  .then(function(r){ return r.json(); })
  .then(function(d){
    if(d.eurusd) updateMercadosDivisa(d);
  }).catch(function(){});
}

/* ── GAS TTF — cada hora ── */
function fetchGasTTF(){
  if(!WORKERS.gasttf) return;
  fetch(WORKERS.gasttf)
  .then(function(r){ return r.json(); })
  .then(function(d){
    if(d.gas_ttf) updateMercadosGas(d);
  }).catch(function(){});
}

/* ── IEA RESERVAS — cada 6h ── */
function fetchReservasIEA(){
  if(!WORKERS.iea) return;
  fetch(WORKERS.iea)
  .then(function(r){ return r.json(); })
  .then(function(d){
    if(d.dias_reserva) updateMercadosReservas(d);
  }).catch(function(){});
}
...
function startAllWorkers(){
  ...
  startBrentTimer();
  startREETimer();
  setTimeout(fetchRiesgoElectrico, 3500);
  setInterval(fetchRiesgoElectrico, 5*60*1000);
  setTimeout(fetchEURUSD, 4000);
  setInterval(fetchEURUSD, 60*60*1000);
  setTimeout(fetchGasTTF, 4500);
  setInterval(fetchGasTTF, 60*60*1000);
  setTimeout(fetchReservasIEA, 5000);
  setInterval(fetchReservasIEA, 6*60*60*1000);
}
